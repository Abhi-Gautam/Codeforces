...
int main()
{
    ...
    {
        in(x)
        if(x > 0) {a.PB(x); suma+=x;}
        else {b.PB(abs(x)); sumb += abs(x);}
        if(i == n-1)
        {
            if(x > 0) last = "first";
            else last = "second";
    ...
    else
    {
        ...
        if(whowon == 0)
        {
            if(a.size() > b.size()) whowon = 1;
            else if(b.size() > a.size()) whowon = 2;
            else
            {
                if(last == "first") whowon = 1;
                else if(last == "second") whowon = 2;
            }
        }
        if(whowon == 1) cout<<"first";
        else cout<<"second";
        // cout<<last;
    }
    return 0;
